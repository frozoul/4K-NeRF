...
'''Model'''
class DirectQVGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
                 ...
                 spatial_pe=0,
                 **kwargs):
        super(DirectQVGO, self).__init__()
        ...
        if rgbnet_dim <= 0:
            ...
            self.dim0 = self.pe_dim + self.k0_dim
            
            self.k0 = grid.create_grid(
                    k0_type, input_dim=self.pe_dim - 3, channels=self.k0_dim, world_size=kwargs['n_cluster'],
                    xyz_min=self.xyz_min, xyz_max=self.xyz_max,
                    config=self.k0_config)
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        if self.rgbnet is None:
            ...
            if self.mode_type == 'TRANS':
                rgb_feat = torch.cat([vq_emb, pe_emb, viewdirs_emb], -1)
                ...
                rgb = torch.sigmoid(rgb_logit_tran)
            elif self.mode_type == 'adain':
                rgb_feat = torch.cat([vq_emb, pe_emb, viewdirs_emb], -1)
                # rgb_feat = vox_emb
