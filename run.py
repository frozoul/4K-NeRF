...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
                      ...
                      eval_ssim=True, eval_lpips_alex=False, eval_lpips_vgg=True, global_step=0, arr_index=None, img_enc=None):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        # torch.save({'o':rays_o, 'd':rays_d, 'vd':viewdirs}, '/mnt/disk1/cuixiao/DirectVoxGO/logs/custom/beef_mlp_z128_dis7e-3_tvd4k5/test_ray.pth')
        ...
        print(f'render 1 frame time is: {time_render}')
        render_result = {
    ...
    return rgbs, depths, bgmaps, psnrs, ssims, lpips_vgg
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if global_step%args.i_val==0:
            ...
            rgbs, depths, bgmaps, psnrs_test, ssim_test, lpips_test = render_viewpoints(
                    model=model,
            ...
            summary_writer.add_scalar('val/ssim',
                            np.mean(ssim_test), global_step=global_step)
            summary_writer.add_scalar('val/lpips',
                            np.mean(lpips_test), global_step=global_step)
            if np.mean(psnrs_test) > psnr_pre:
                psnr_pre = np.mean(psnrs_test)
                psnr_path = os.path.join(cfg.basedir, cfg.expname, f'best_psnr.tar')
                torch.save({
...
if __name__=='__main__':
    ...
    if args.render_train:
        ...
        rgbs, depths, bgmaps, _, _, _ = render_viewpoints(
                ...
                **render_viewpoints_kwargs)
        imageio.mimwrite(os.path.join(testsavedir, 'video.rgb.mp4'), utils.to8b(rgbs), fps=25, quality=8)
    ...
    if args.render_video:
        ...
        rgbs, depths, bgmaps, _, _, _ = render_viewpoints(
                ...
                eval_ssim=False, eval_lpips_alex=False, eval_lpips_vgg=False,
                **render_viewpoints_kwargs)
        imageio.mimwrite(os.path.join(testsavedir, 'video.rgb.mp4'), utils.to8b(rgbs), fps=25, codec='libx264', quality=8)
